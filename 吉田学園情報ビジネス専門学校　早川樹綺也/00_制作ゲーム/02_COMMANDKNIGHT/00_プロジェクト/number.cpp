...
// 静的メンバ変数宣言
//=============================================================================
LPDIRECT3DTEXTURE9 CNumber::m_pTexture = NULL;	//テクスチャへのポインタ情報
...
{
	CNumber *pNumber = NULL;	//数字の動的確保
	if (pNumber == NULL)
	{
		pNumber = new CNumber;				//シーンの動的確保
		if (pNumber != NULL)
		{
			pNumber->SetPos(pos);			//位置の設置処理
			pNumber->SetWidth(fWidth);		//幅の設置処理
			pNumber->SetHeight(fHeight);	//高さの設置処理
			pNumber->Init();				//初期化処理
		}
	}
	return pNumber;					//値を返す
